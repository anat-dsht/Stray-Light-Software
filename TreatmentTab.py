...
class StrayLightTreatment(VisualisationTab):
    ...
    def __init__(self, initialize_connections=False):
        ...
        self.pico_scrollbox: QComboBox = QComboBox()
    ...
    def setup_traitement_ui(self):
        ...
        self.parameter_layout.addWidget(self.pico_scrollbox, 2, 1)
        ...
        removewidget(self.parameter_layout, self.pico_scrollbox_visualisationtab)
        ...
        # Define possibilities for the pico choice
        picos = ["pico 1", "pico 2", "pico 3", "pico 4", "pico 5"]
        for pico in picos:
            self.pico_scrollbox.addItem(pico)
    ...
    def call_functions(self):
        ...
        for file_path in file_paths:
            ...
            self.get_graph_parameters(file_path, self.pico)
    ...
    def call_functions_other_spectrum(self, files):
        ...
        for file in files:
            ...
            if 'dark' in file.lower():
                ...
                # Get the canal, linked data and legends for the current scan treated
                self.get_graph_parameters_scan0P(file, 1)
                ...
                # Get the canal, linked data and legends for the current scan treated
                self.get_graph_parameters_scan0(file, 1)
                self.record_type = add_legends(file)
    ...
    def extract_datas(self, file_path):
        ...
        # Call the function to retrieve pico number selected by user
        self.traduce_pico_choice()
        ...
        for folder in parent_folder.iterdir():
            if folder.is_dir() and "noscan" in folder.name.lower():
                ...
                if processed_folder.exists() and processed_folder.is_dir():
                    ...
                    # Check for the presence of files in the folder
                    if processed_files:
                        if self.pico is not None:
                            # Retrieve pico number selected by user
                            pico_num = int(self.pico.split()[1])
                        else:
                            # Handling the case where pico is None
                            pico_num = -1
                            print("Error: pico is not defined.")

                        # Check that you have enough files
                        if pico_num <= len(processed_files):
                            new_file_path_noscan.append(str(processed_files[pico_num - 1]))
                        else:
                            print(f"Not enough files for Pico {pico_num}")
            ...
            if folder.is_dir() and "dark" in folder.name.lower():
                ...
                if processed_folder.exists() and processed_folder.is_dir():
                    ...
                    if processed_files:
                        ...
                        # Check that you have enough files
                        if pico_num <= len(processed_files):
                            new_file_path_dark.append(str(processed_files[pico_num - 1]))
                        else:
                            print(f"Not enough files for Pico {pico_num}")
    ...
    def traduce_pico_choice(self) -> object:
        """
        Get pico number of the chosen signal.

        :return: int
        """

        # Pico currently selected by the user
        pico_choice = self.pico_scrollbox.currentText()

        # Get pico number
        for pico in range(1, 6, 1):
            if pico_choice == f"pico {pico}":
                self.pico = "Pico " + str(pico)
    ...
    def process_files(self, files):
        ...
        for filename in files:
            ...
            target_file = None
            for file in files:
                if f"co{str(self.pico[-1])}" in file:
                    target_file = file
                    break
                elif f"calc{str(int(self.pico[-1]) - 1)}" in file:
                    target_file = file
                    break
