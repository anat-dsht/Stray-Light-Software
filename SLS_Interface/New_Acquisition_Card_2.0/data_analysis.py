...
class DataAnalysis(QWidget):
    ...
    def compare_pos_plot(self, xdata: object, ydata: object, color: object, title: object, x_label: object,
        ...
        if self.plot_id <= graph_id:
            ...
            self.ax = self.figure.add_subplot(int(f"1{AMOUNT_OF_GRAPHS}{self.plot_id}"))
        ...
        try:
            ...
            for data in range(int(nb_or_rounds - 1), len(xdata)):
                sns.scatterplot(x=xdata[data], y=ydata[data], ax=self.ax, color=color[data], label=legend[data][14:])
        ...
        try:
            if xdata is not None and ydata is not None :
                ...
                if graph_id == 1:
                    ...
                    if "validity limit" not in labels:
                ...
                try:
                    ...
                    # Browse the new data to plot
                    for data in range(int(nb_or_rounds - 1), len(xdata)):
                        sns.scatterplot(x=xdata[data], y=ydata[data], ax=self.ax, color=color[data], label=legend[data][:13])
            ...
            raise Exception(f"Unexpected Error in compare_pos_plot_calibrator: {e}") from e
