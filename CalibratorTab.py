...
from VisualisationTab import (VisualisationTab, add_legends, OPD_BANDWIDTH, FREQ_BANDWIDTH, LOW_OPD_BANDWIDTH,
                              LOW_FREQ_BANDWIDTH, LOW_OPD_BANDWIDTH_NOISE_MODEL)
...
class CalibratorTab(StrayLightTreatment):
    ...
    def setup_calibrator_ui(self):
        ...
        self.plot_options_layout.addWidget(self.checkbox_data_plotline_calibrator, 2, 0)
        ...
        removewidget(self.parameter_layout, self.pico_scrollbox)
        removewidget(self.parameter_layout, self.checkbox_plot_options_text)
        removewidget(self.plot_options_layout, self.checkbox_data_plotline)
        removewidget(self.plot_options_layout, self.checkbox_treated_signal)
        removewidget(self.layout, self.button_access_data)

        # Remove element which were defined in the Data Visualisation tab
        removewidget(self.parameter_layout, self.pico_text)
    ...
    def call_functions_calibrator(self):
        ...
        if "Scan51" in file_path1:
            # On recherche un fichier contenant "Scan51" dans le répertoire self.ref_file_path
            for file_name in os.listdir(self.ref_file_path):
                if "51" in file_name:
                    file_path2 = os.path.join(self.ref_file_path, file_name)
                    break  # On arrête la boucle dès qu'on trouve le fichier correspondant

        # Si "Scan53" est dans file_path1, on définit file_path2
        elif "Scan53" in file_path1:
            # On recherche un fichier contenant "Scan53" dans le répertoire self.ref_file_path
            for file_name in os.listdir(self.ref_file_path):
                if "53" in file_name:
                    file_path2 = os.path.join(self.ref_file_path, file_name)
                    break  # On arrête la boucle dès qu'on trouve le fichier correspondant
        ...
        for file_path in file_paths:
            ...
            self.get_graph_parameters_calibrator(file_path, 5)
            ...
            if "OPD" not in self.x_axis_legend:
                self.treatment_method_calibrator(FREQ_BANDWIDTH) & self.low_treatment_method_calibrator(LOW_FREQ_BANDWIDTH, file_path)
            else:
                self.treatment_method_calibrator(OPD_BANDWIDTH) & self.low_treatment_method_calibrator(LOW_OPD_BANDWIDTH, file_path)
    ...
    def low_treatment_method_calibrator(self, bandwidth, file_path):
        """
        ...
        self.log_fit_amplitude_optimal_calibrator, self.fit_amplitude_optimal_calibrator = (
            self.create_noise_model_calibrator(LOW_OPD_BANDWIDTH_NOISE_MODEL, self.low_freqs_calibrator,
                                               self.low_data_calibrator, self.log_freqs_calibrator,
                                               self.log_data_calibrator))
        ...
        elif self.count_calibrator % 2 != 0:
    ...
    def create_noise_model_calibrator(self, bandwidth, freqs, data, log_freqs, log_data):
    ...
    def get_custering_low_variables_calibrator(self, noise_floor_values):
        ...
        low_xpeak, low_ypeak, _ = self.detect_peaks_above_noise_model_calibrator(noise_floor_values,
                                                                    self.low_freqs_calibrator, self.low_data_calibrator)
        self.low_barycenters_calibrator, self.low_barycenter_clusters_calibrator, self.low_full_clusters_peaks_calibrator \
    ...
    def detect_peaks_above_noise_model_calibrator(self, fit_amplitude_optimal, low_freqs, low_data, threshold_factor=3,
                                                  window_size=12):
        """
    ...
    def scans_comparison_calibrator(self):
        ...
        self.barycenter_processor_calibrator = ScansComparison(
            ...
            self.x_ratio_values_calibrator, self.y_ratio_values_calibrator, self.y_max_values_calibrator, self.x_max_values,
            self.scans_done_calibrator, self.freqs_calibrator, self.freqs_calibrator_51, self.data_calibrator,
            self.data_calibrator_51,
            self.list_straylight_barycenter51_calibrator, self.list_perturbation_barycenter51_calibrator,
            self.list_straylight_barycenter53_calibrator, self.list_perturbation_barycenter53_calibrator,
            self.pic_number_calibrator, self.peak_width_51_calibrator, self.peak_width_53_calibrator, self.quality_x_SL,
            self.quality_y_SL, self.quality_x_perturbation, self.quality_y_perturbation, self.list_straylight_max51,
            self.list_perturbation_max51, self.list_straylight_max53, self.list_perturbation_max53)
    ...
    def low_scans_comparison_calibrator(self):
        ...
        self.barycenter_processor_calibrator = ScansComparison(
            ...
            self.low_scans_comparison_treated_signal_y_calibrator_51,
            self.low_x_ratio_values_calibrator, self.low_y_ratio_values_calibrator, self.low_y_max_values_calibrator,
            self.low_x_max_values, self.low_scans_done_calibrator, self.low_freqs_calibrator,
            self.low_freqs_calibrator_51, self.low_data_calibrator, self.low_data_calibrator_51,
            self.low_list_straylight_barycenter51_calibrator,
            ...
            self.low_list_straylight_max51,
            self.low_list_perturbation_max51, self.low_list_straylight_max53, self.low_list_perturbation_max53)

        (low_list_y_max, low_indices_to_remove_calibrator, self.low_sl_points_y_calibrator, self.low_sl_points_x_calibrator,
            self.low_scans_comparison_treated_signal_x_calibrator, self.low_scans_comparison_treated_signal_y_calibrator,
            self.low_x_ratio_values_calibrator, self.low_y_ratio_values_calibrator, self.low_y_max_values_calibrator,
            self.low_scans_done_calibrator, self.low_list_straylight_barycenter51_calibrator, self.low_list_perturbation_barycenter51_calibrator,
            self.low_list_straylight_barycenter53_calibrator, self.low_list_perturbation_barycenter53_calibrator,
            self.low_pic_number_calibrator) = (
            self.barycenter_processor_calibrator.compare_barycenters_calibrator(X_CONDITION_BARYCENTER_COMPARISON,
                                                                                Y_CONDITION_BARYCENTER_COMPARISON))
    ...
    def create_plot_calibrator(self, xaxis: object, yaxis: object, xlabel: object, ylabel: object, canal: object, file_path) -> object:
        """
        ...
        if self.checkbox_barycenters.isChecked():
            ...
            if all_barycenters.size > 0:
                handle = sns.scatterplot(x=all_barycenters[:, 0], y=all_barycenters[:, 1], ax=self.ax,
                                             ...
                                             label=f"Barycenter ({POINTS_CALCUL_BARYCENTRE}pts) {self.record_type_calibrator[:8]}")
            else:
        ...
        if self.checkbox_noise_floor.isChecked():
            ...
            sns.lineplot(x=self.low_freqs_calibrator, y=self.fit_amplitude_optimal_calibrator,
    ...
    def show_filtered_spectrum_calibrator(self, file_path):
        ...
        def add_plot_calibrator(x_data, y_data, x_label, y_label, canal, file_path):
            """
            ...
            self.create_plot_calibrator(x_data, y_data, x_label, y_label, canal, file_path)
        ...
        add_plot_calibrator(pd.Series(self.freqs_tot_calibrator), pd.Series(self.data_tot_calibrator),
                            self.x_axis_legend, self.y_axis_legend, self.canal, file_path)
    ...
    def overlay_filtered_plot_calibrator(self, file_path=None):
        ...
        def add_overlay_plot_calibrator(x_data, y_data, canal, file_path=None):
            ...
            if not self.scans_done_calibrator:
                ...
                if self.checkbox_noise_floor.isChecked():
                    ...
                    sns.lineplot(x=self.low_freqs_calibrator, y=self.fit_amplitude_optimal_calibrator, ax=self.ax,
                                     color=self.line_color)
        ...
        # Call of functions to plot in the data_analysis tab
                ...
                self.data_analysis_tab_calibrator.compare_pos_plot_calibrator(self.pic_number_calibrator,
                                                                        self.y_ratio_values_calibrator,
                                                                        self.plot_colors,
                                                                        'y_ratio for each calibrator peak',
                                                                        'Peak',
                ...
                self.data_analysis_tab_calibrator.compare_pos_plot_calibrator(self.pic_number_calibrator,
                                                                        self.x_ratio_values_calibrator,
                                                                        self.plot_colors,
                                                                        'x_ratio for each calibrator peak',
                                                                        'Peak',
